...
#
# 节点主机的配置
IMAGE_ID=ami-062a49a8152e4c031          #   选择的镜像，此处为 Amazon Linux 2023
...
SECURITY=sg-0efc5d37958e05d4d           #   安全分组，决定防火墙规则
REGION=eu-west-1                        #   实例所处地区
#
...
deploy_function() {
    ...
    echo "Starting $num server in $REGION"
    ...
    new_instance_info=$(aws ec2 run-instances \
        ...
        --output json)
    echo "$new_instance_info" | jq '.' > west/createResult.json
    rm west/createResult.json
